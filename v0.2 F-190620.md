...
　　　什么 你说[BD](https://fr.wikipedia.org/wiki/Bande_dessin%C3%A9e)生肉？文件名不再有用？那么[Yggtorrent](https://www.yggtorrent.ch)适合你，也可以看我的书签。
...
个人常用搜索的URL，可添加到各类自定义搜索中。从地址可知网站，amazon第二个为kindle商店。

`{q}`是wox写法，请替换为浏览器写法，如`%s`



```
https://book.douban.com/subject_search?search_text={q}
http://book.ucdrs.superlib.net/search?sw={q}&allsw=&bCon=&ecode=utf-8&channel=search&Field=all
https://www.yunpanjingling.com/search/{q}?utm_source=extension
https://www.amazon.cn/s?k={q}
https://www.amazon.cn/s?k={q}&i=digital-text&__mk_zh_CN=%E4%BA%9A%E9%A9%AC%E9%80%8A%E7%BD%91%E7%AB%99&ref=nb_sb_noss
http://xueshu.baidu.com/s?wd={q}&rsv_bp=0&tn=SE_baiduxueshu_c1gjeupa&rsv_spt=3&ie=utf-8&f=8&rsv_sug2=1&sc_f_para=sc_tasktype%3D{firstSimpleSearch}&rsv_n=2
http://book.gzlib.org/search?Field=all&channel=search&sw={q}&ecode=utf-8
http://readfree.me/search/?q={q}
http://book.duxiu.com/search?sw={q}&allsw=&searchtype=&channel=search&bCon=&ecode=utf-8&Field=all
https://www.mlook.mobi/search?q={q}
https://www.panc.cc/s/{q}/td_0
https://www.xiaobd.net/m/search?wd={q}
```
...
　　导出图片、PitStop、pdftoy流过滤、`ComicEnhancerPro`（下文略为`CEP`）[PDF水印清理专家](http://www.liyusoft.com/pdfwr)等
...
　　若是台版书，PDF阅读器很少支持从左到右翻页。安卓支持的多，详见PDF—[移动平台](#%E7%A7%BB%E5%8A%A8%E5%B9%B3%E5%8F%B0)。
...
　　通常院校会购买机构授权，如[EndNote](https://www.endnote.com/)、[NoteExpress](http://www.inoteexpress.com/aegean/)，下载亦不难找。由于大家需求多样、差异较大，本人经验有限，因此仅列举并简要说明。类似的知识管理软件如[印象笔记](https://www.yinxiang.com/)、[为知笔记](http://www.wiz.cn/)因长期不再使用，无法推荐。
...
　　作者目前使用的书签，**个人痕迹明显、杂乱不全**……还有太多未整理没精力添加（咱现在都是搜书签）

　　特色是还算全的在线漫画站点、下载生肉的博客；能在线看的期刊、数据可视化和一点儿开放教科书
...
即[上文链接](https://weibo.com/ttarticle/p/show?id=2309404181872988011638)的doc版本
...
　　如《lshort中文版4.2》闪退，但5.0-6.0.1版本均无问题，目前推测与部分CTeX输出文档存在兼容问题。

　　由于CTeX套装已停止维护，其输出的版本现存极少、难以找到合适的档案打开/输出测试，且本人XeTeX 3.14159265-2.6-0.99999(TeX Live 2018/W32TeX) 输出的PDF没有任何问题……因此不再深入兼容测试。



#### 二、安卓PDF阅读器

　　日常Adoobe足够，兼容性好。但功能简陋，如不支持裁剪、底色，不太适合大量阅读。

　　只要是支持PDF的阅读器，表现都不错…但也有区别，如下：

1. 特大文件　300M以上　可拿扫描版辞典与[书格古籍](https://www.shuge.org/ebooks/)试验。
2. 加载模式　多指图片PDF如何完全显示。有时也包含复杂的文字版、能否模拟翻页。
   - 瀑布式逐行：Adobe Reader
   - 模糊变清晰：XoDo、WPS PDF
   - 空白/进度条：多看、智器 、SideBooks
3. 翻页跳转　快速翻动、拖动进度跳转时候加载所需时间
4. 编码兼容　多种中文文字版PDF能否正常打开，仅少数软件支持不行
5. 附属功能　标注笔记、文件底色、转换合并、朗读等，其中底色修改能够对白色替换的不多。

　　可见强调个人需求、习惯，因此不做过多介绍，请参考[酷安应用集](https://www.coolapk.com/album/12176502)的文末、回复区。



#### 三、安卓CHM阅读器

　　由于CHM文件年代久远、制作不规范等问题，很难做到较好的兼容性。
...
？：掌阅我无法翻页，能正确显示第一页标记为此。（除9外估计没问题，也就是成功7个）
