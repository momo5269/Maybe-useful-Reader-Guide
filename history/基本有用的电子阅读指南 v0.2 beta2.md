...
​		今后仅提供`.md`文件，阅读和输出PDF/HTML请使用[`Typora`](https://www.typora.io/)  推荐[VUE](http://theme.typora.io/theme/Vue/)主题。
...
​		于是有了速览版，点击即可进入下载页。（不含实用工具，安卓应用可能需要翻墙）
...
​		各平台和在线词典有很多，此处主要推荐繁体/生僻/异体字查询、轻便的客户端，木有MAC和某些著名跨平台软件。亦不提供插图等输入方案（可以百度）和罗塞塔石碑等学习软件。另外，异体字多需要语境参考，未有一定水平不建议深入。

​		[表意文字描述字符](http://zh.wikipedia.org/zh-hk/表意文字序列#.E8.A1.A8.E6.84.8F.E6.96.87.E5.AD.97.E5.AD.97.E7.AC.A6)可参考[如何精确地表示生僻字](https://zhuanlan.zhihu.com/p/19753292) 
...
FreeMat、Isabelle、MathType、Mathematics、计算工厂、OriginLab
...
包含代码支持，但不含`十六进制`/`代码整理`/`编程辅助`相关工具。

EmEditor、notepad++、notepad2、VSCode、Sublime Text、EditPlus、emacs、atom、BowPad
...
此类软件太多，大家需求不一，推荐起来太难……于是仅简单推荐几个，不含在线工具。
...
> 截图	[FastStoneCapture](http://blog.sina.com.cn/s/blog_89a729a40102wjwk.html)





截图	Greenshot、Snipaste、WinSnap、AeroSnap、PicPick

截图OCR	PandaOCR	天若 OCR	树洞 OCR 	NetOCR	QQ截图+OCR	简谱转调	mathpix_snipping_tool	cencrack识别图片转文字	Capture2Text	移动平台 QQ、微软的翻译等等

GIF	ScreenToGif、GifCam、licecap

录像	oCam、Bandicam、屏幕录象专家、系统自带、MSIAfterburner

其他	诸如Apowersoft Screen Recorder 等，不再赘述。 
...
​			因为面向初学者，最多是修订小错，除文本编辑器外，只简单介绍几个常用工具。

[`mkvtoolnix`](https://mkvtoolnix.download/)+[`gMKVExtractGUI`](https://sourceforge.net/projects/gmkvextractgui/)	MKV工具及其导出程序的GUI，用以提取字幕。

[`BatchSubtitlesConverter`](https://sites.google.com/site/tsdarkness/batchsubtitlesconverter)	优秀的字幕格式转换，常用于`SRT`转`ASS`。新版本收费，免费的[1.24版](https://sites.google.com/site/tsdarkness/batchsubtitlesconverter/Download)够用。
...
搜索工具Everthing、docfetcher、FileLocator、Listary

重命名ReNamer、Advanced Renamer、深蓝批量重命名、拖把更名器

文件列表文件列表生成器、DirLister

重复查询Duplicate Cleaner、Beyond Compare 4

文件同步FreeFileSync、Allway Sync 'n' Go

编码转换CaptfEncoder

快捷启动WOX、 Rolan、Lily、Launchy、uTools、RunAny、nTrun
