...
​		其他如[Scankromsator](https://www.52pojie.cn/thread-831415-1-1.html)也不错，但上手很难。而`BookRestorer`和`RecogniformImage`等仅英文，适用少、有兼容问题。
...
​		我改meta是因为处理后等于新版本，收尾加个也不碍事。同时方便使用filemeta管理的同学检索。这种事情，若非花费大量时间的确不需要标注防偷…真有。用老马的软件生成双层PDF时一般`制作程序`不作变动。
